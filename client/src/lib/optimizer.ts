  ...
  private buildTieBreakModel(
    ...
    for (let i = 0; i < N; i++) {
      for (let s = 0; s < S; s++) {
        ...
        allDeviationVars.push(`ds_${i}_${s}`);
      }
    }
    ...
    if (allDeviationVars.length > 0) {
      const devTerms = allDeviationVars.map((v, idx) => idx === 0 ? v : `+ ${v}`);
      constraintLines.push(`  c${cIdx.val++}:`);
      constraintLines.push(writeTerms(devTerms, 10));
      constraintLines.push(`  <= ${fmt(bestDeviation)}`);
    }
    ...
    let firstTerm = true;
    for (let i = 0; i < N; i++) {
      objParts.push(`${firstTerm ? "" : "+ "}${fmt(1e-6)} tw_${i}`);
      firstTerm = false;
    }
    if (enableHolidayBalance && staffHolidayTargets.some(t => t > 0)) {
      for (let i = 0; i < N; i++) {
    ...
    if (coverageRatio < this.coverageThreshold) {
      ...
      if (phase2aSolution && bestRange < Infinity) {
        ...
        try {
          ...
          if (finalSolution === phase2bSolution && phase2bSolution.Columns) {
            let bestDeviation = 0;
            const cols2b = phase2bSolution.Columns;
            for (const key in cols2b) {
              if (key.startsWith("ds_")) {
                bestDeviation += cols2b[key].Primal || 0;
              }
            }
            console.log(`[OPT] Phase 2B best deviation: ${bestDeviation}`);

            try {
              const tieModel = this.buildTieBreakModel(varMap, binaryVars, phase1Targets, bestRange, bestDeviation);
              console.log(`[OPT] Phase 2C (tie-break) model: ${tieModel.length} chars, ${tieModel.split('\n').length} lines`);

              const solver2c = await createSolver();
              const phase2cSolution = solver2c.solve(tieModel, {
                time_limit: 60,
                mip_rel_gap: 1e-4,
                mip_abs_gap: 1e-6,
                threads: 1,
              ...
              if (failStatuses.includes(phase2cSolution.Status) || !phase2cSolution.Columns) {
              ...
              console.error("[OPT] Phase 2C CRASH, keeping Phase 2B result:", String(err));
