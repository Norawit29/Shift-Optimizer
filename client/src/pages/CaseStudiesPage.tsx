    ...
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      <main className="pt-28 pb-16 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto">
          <div className="mb-10">
            <h1 className="text-3xl sm:text-4xl font-display font-bold text-slate-900 dark:text-white mb-3" data-testid="text-case-studies-title">
              {t.caseStudiesTitle}
            </h1>
            <p className="text-lg text-slate-600 dark:text-slate-400" data-testid="text-case-studies-subtitle">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 text-slate-500 dark:text-slate-400">
              <p className="text-lg" data-testid="text-case-studies-empty">{t.caseStudiesEmpty}</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6" data-testid="case-studies-list">
              {caseStudies.map((cs) => (
                <Link
                  key={cs._id}
                  href={`/case-studies/${cs.slug?.current || ""}`}
                  className="block group"
                  data-testid={`case-study-card-${cs._id}`}
                >
                  <div className="h-full rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden transition-shadow duration-300 hover:shadow-lg hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50 bg-white dark:bg-slate-900 flex flex-col">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      <h2 className="text-base font-bold text-slate-900 dark:text-white mb-2 group-hover:text-primary transition-colors line-clamp-2" data-testid={`case-study-title-${cs._id}`}>
                        {cs.title}
                      </h2>
                      {cs.excerpt && (
                        <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed mb-3 line-clamp-2 flex-1" data-testid={`case-study-excerpt-${cs._id}`}>
                          {cs.excerpt}
                        </p>
                      )}
