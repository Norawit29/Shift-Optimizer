...
- **Optimization Details**: The optimizer is fully deterministic. Phase 1/2A: time_limit=90s, mip_rel_gap=0.001. Phase 2B: adaptive config (≤40 staff: 90s/1e-4, ≤80: 120s/5e-4, >80: 90s/1e-3), mip_abs_gap=1e-6. Workload fairness: `dev_i` variables penalize deviation from average load. Weights: LOAD_W=100, SHIFT_W=20, HOLIDAY_W=20 (LEVEL_W >> LOAD_W >> SHIFT_W). Shift target remainder allocation is randomized to prevent index bias. Range precision: Math.round(rawRange + 1e-6) instead of Math.ceil. Worker timeout: 210s. Greedy post-processing fills remaining slots respecting all hard constraints. `checkLevelFillability()` runs pre-solve to detect level shortages for diagnostics.
