...
- **Core Functionality**: The shift optimization algorithm runs client-side within a Web Worker (`solverWorker.ts`) to maintain UI responsiveness. It uses the HiGHS solver (WASM) with a 3-stage Mixed Integer Programming (MIP) pipeline.
    - **Phase 1 (Coverage)**: `buildCoverageModel()` — Maximizes total coverage (minimizes unfilled slots). Includes staffing capacity, maxShifts, shiftsPerDay, consecutiveRules, maxConsecutiveRules, max 7 consecutive working days (built-in), and requested shifts. Excludes level constraints and fairness.
    - **Phase 1.5 (Quality Check)**: If coverage ratio < 20%, skips fairness phase and returns partial result with diagnostics.
    - **Phase 2A (Max-Min)**: `buildMaxMinModel()` — Locks coverage from Phase 1 (cannot reduce). Maximizes minLoad (lexicographic fairness). No per-shift balance, holiday balance, or level constraints.
    - **Phase 2B (Min-Max)**: `buildMinMaxModel()` — Locks minLoad from Phase 2A (`minLoad >= bestMinLoad`). Minimizes maxLoad only. No deviation terms.
    - **Phase 2C (Distribution)**: `buildDistributionModel()` — Locks both minLoad and maxLoad from Phase 2A/2B. Minimizes per-shift balance (ds_*, weight 1000) + holiday balance (dh_*, weight 100) + soft level slack (lslk_*, weight 10). Level constraints added as soft only. `writeSoftLevelConstraints()` generates the soft level constraint block.
