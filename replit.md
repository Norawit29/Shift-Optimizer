...
The shift optimization algorithm runs **client-side** via `runOptimizerInWorker()` (in `optimizer.ts`) which wraps the optimizer call in a `setTimeout(0)` to yield to the browser before heavy computation, keeping CSS animations (compositor-thread) smooth. The optimizer uses **Mixed Integer Programming (MIP)** via the HiGHS solver (WASM) with a **2-phase approach**:
